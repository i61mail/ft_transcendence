...
export function pongOnline(player1 : intf.playerInfo, player2 : intf.playerInfo)
{
    ...
    let pong: PongGame = new PongGame(intf.GameMode.online, player1, player2);
}

export function pongLocal(player : intf.playerInfo)
{
    player.socket.send(JSON.stringify({gm: intf.GameMode.local, plyI: 0}));
    let pong: PongGame = new PongGame(intf.GameMode.local, player);
}

export function pongAI(player : intf.playerInfo, difficulty: intf.Difficulty)
{
    player.socket.send(JSON.stringify({gm: intf.GameMode.local, plyI: 0}));
    let pong: PongGame = new PongGame(intf.GameMode.AI, player, undefined, difficulty);
}
