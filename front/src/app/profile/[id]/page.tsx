...
export default function UserProfilePage()
{
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#c8d5e8] via-[#bcc3d4] to-[#a8b0c5]">
        <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-12 shadow-2xl border border-white/20">
          <p className="font-pixelify text-lg text-[#2d5a8a] font-bold">Loading profile...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    ...
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#c8d5e8] via-[#bcc3d4] to-[#a8b0c5]">
        <Header user={user} onUserUpdate={setUser} activeRoute="chat" />
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-12 w-full max-w-6xl shadow-2xl border border-white/20">
            <div className="text-center">
              <p className="text-6xl mb-6">👤</p>
              <p className="font-pixelify text-3xl font-bold text-[#2d5a8a] mb-3">
                {isUserNotFound ? "User Not Found" : "Unable to Load Profile"}
              </p>
              <p className="font-pixelify text-lg text-[#2d5a8a]/70 mb-8">
                {isUserNotFound 
              ...
              <button
                onClick={() => router.back()}
                className="font-pixelify backdrop-blur-md bg-white/30 hover:bg-white/40 text-[#2d5a8a] font-bold py-3 px-8 rounded-full transition-all duration-300 shadow-lg border border-white/50 hover:scale-105"
              >
                Go Back
              </button>
            </div>
  ...
  if (!profile) {
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#c8d5e8] via-[#bcc3d4] to-[#a8b0c5]">
        <Header user={user} onUserUpdate={setUser} activeRoute="chat" />
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-12 w-full max-w-6xl shadow-2xl border border-white/20 text-center">
            <p className="font-pixelify text-2xl text-[#2d5a8a] font-bold">👤 Profile not found</p>
          </div>
        </div>
      </div>
    );
      ...
      <main className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-8 w-full max-w-6xl shadow-2xl border border-white/20 max-h-[calc(100vh-140px)] overflow-y-auto">
          <div className="flex items-center gap-4 mb-6">
            <button
              onClick={() => router.back()}
              className="font-pixelify text-2xl backdrop-blur-md bg-white/20 hover:bg-white/30 rounded-full w-12 h-12 flex items-center justify-center transition-all duration-300 border border-white/30 text-[#2d5a8a] font-bold hover:scale-110"
            >
              ←
            </button>
            <h1 className="font-pixelify text-4xl font-bold text-[#2d5a8a] drop-shadow-lg">
