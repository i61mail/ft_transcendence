...
const ChatPreview = (data: Props) => {
      ...
      setTimeout(()=>
        {
          if (
            manager.status &&
            manager.status.id === data.friend.friend_id
          )
            setStatus(manager.status.status);
        }
        ,200);
    }
  }, [manager.status]);
  ...
  useEffect(() =>
  {
    if (manager.latestMessage && manager.latestMessage.friendship_id === data.friend.id && manager.pointedUser?.id !== data.friend.id)
